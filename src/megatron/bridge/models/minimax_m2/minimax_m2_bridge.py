...
class MiniMaxM2Bridge(MegatronModelBridge):
    ...
    scoring and expert bias correction). MiniMax-M2.5 uses the same
    ``MiniMaxM2ForCausalLM`` architecture / ``model_type=minimax_m2`` config
    family, so this bridge supports both checkpoints. Use the native
    transformers >= 5.0 implementation when available (no ``trust_remote_code``
    required).
    ...
    Example:
        ...
        >>> bridge = AutoBridge.from_hf_pretrained("MiniMaxAI/MiniMax-M2.5")
