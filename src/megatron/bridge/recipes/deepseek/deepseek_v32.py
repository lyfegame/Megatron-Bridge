...
"""
DeepSeek V3.2 Training Recipe.

V3.2 introduces DSA (DeepSeek Sparse Attention) which enables O(L*K) attention
complexity instead of O(L^2) by selecting top-K tokens per query position.
...
def set_deepseek_v32_pipeline_model_parallel_layout(
    ...
    """Set the DeepSeek-V3.2 pipeline model parallel layout.

    V3.2 has the same 61 decoder layers as V3, so the layout is identical.
    """
...
class DeepSeekV32CommonKwargs(TypedDict, total=False):
    """Typed options accepted by DeepSeek V3.2 recipe helper functions."""
    ...
    # DSA (DeepSeek Sparse Attention) configuration - V3.2 specific
    dsa_indexer_n_heads: Optional[int]
    dsa_indexer_head_dim: Optional[int]
    dsa_indexer_topk: Optional[int]
    dsa_indexer_loss_coeff: Optional[float]
    dsa_indexer_use_sparse_loss: Optional[bool]
...
def deepseek_v32_pretrain_config(**user_kwargs: Unpack[DeepSeekV32CommonKwargs]) -> ConfigContainer:
    """Return a pre-training config for DeepSeek-V3.2.
    ...
    """
    recommended_kwargs: DeepSeekV32CommonKwargs = {
    ...
    combined_kwargs: DeepSeekV32CommonKwargs = {**recommended_kwargs, **user_kwargs}
    return _deepseek_v32_common(**combined_kwargs)


def deepseek_v32_pretrain_config_32nodes(**user_kwargs: Unpack[DeepSeekV32CommonKwargs]) -> ConfigContainer:
    """
    Create a pre-training configuration for DeepSeek-V3.2 model with minimal number of nodes (32).
    ...
    combined_kwargs: DeepSeekV32CommonKwargs = {**recommended_kwargs, **user_kwargs}
    return deepseek_v32_pretrain_config(**combined_kwargs)


def _deepseek_v32_common(
    hf_path: str,
    ...
    # DSA (DeepSeek Sparse Attention) configuration - V3.2 specific
    dsa_indexer_n_heads: Optional[int] = None,  # Default from HF config (typically 64)
    dsa_indexer_head_dim: Optional[int] = None,  # Default from HF config (typically 128)
    dsa_indexer_topk: Optional[int] = None,  # Default from HF config (typically 2048)
    dsa_indexer_loss_coeff: Optional[float] = None,  # KL divergence loss coefficient
    dsa_indexer_use_sparse_loss: Optional[bool] = None,  # Use sparse loss with top-k indices
    # Training hyperparameters
    ...
    """
    Create a pre-training configuration for DeepSeek-V3.2 models using a given HuggingFace path.
    ...
    # V3.2 DSA (DeepSeek Sparse Attention) configuration
    # Enable DSA attention variant
    model_cfg.experimental_attention_variant = "dsa"
    # RoPE fusion is not supported with DSA
    model_cfg.apply_rope_fusion = False

    # Set DSA indexer parameters if provided (otherwise use HF config defaults)
    if dsa_indexer_n_heads is not None:
        model_cfg.dsa_indexer_n_heads = dsa_indexer_n_heads
    if dsa_indexer_head_dim is not None:
        model_cfg.dsa_indexer_head_dim = dsa_indexer_head_dim
    if dsa_indexer_topk is not None:
        model_cfg.dsa_indexer_topk = dsa_indexer_topk
    if dsa_indexer_loss_coeff is not None:
        model_cfg.dsa_indexer_loss_coeff = dsa_indexer_loss_coeff
    if dsa_indexer_use_sparse_loss is not None:
        model_cfg.dsa_indexer_use_sparse_loss = dsa_indexer_use_sparse_loss

    set_deepseek_v32_pipeline_model_parallel_layout(model_cfg, layout)
    ...
    # Note: RoPE fusion is disabled for DSA, so we don't enable experimental flag
    # cfg.dist.enable_megatron_core_experimental = True  # Not needed
